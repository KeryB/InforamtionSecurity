package ru.information.security.utils;

public class UserUtils {

    public static final String ADMIN = "Администратор";

    public static final String USER = "Пользователь";

    public static final String ADMIN_ICON_PATH = "src/main/resources/icons/admin.png";

    public static final String ROLE = "Роль";

    public static final String LOGIN = "Логин";

    public static final String IS_BLOCKED = "Блокировка";

    public static final String CONSTRAINS = "Парольные ограничения";

    public static final String BLOCKED_USER_BODY = "Вы были заблокрованы админом, поэтому вы ничего не можете делать";

    public static final String BLOCKED_USER_HEADER = "Блокировка пользователя";

    public static final String EMPTY_PASSWORD_BODY = "У вас обнаружен пустой пароль, поменяйте пароль для дальнейшей работы в системе";

    public static final String EMPTY_PASSWORD_HEADING = "Внимание!";
}
